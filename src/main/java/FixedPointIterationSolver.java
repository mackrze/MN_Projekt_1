...
public class FixedPointIterationSolver {
    private Function function;

    public FixedPointIterationSolver(Function function) {
        this.function = function;
    }

    public ArrayList<double[]> solver(double xL, double xU, double ea, double e,double M,double xn0) { //start solver
        ArrayList<double[]> solver = new ArrayList<>();
        double[] solve = new double[4];
        solve[0] = xL;
        solve[1] = xU;
        solve[2] = calculateXR(xn0,e,M);
        solve[3]=100;
        solver.add(solve);
    ...
    private double calculateXR(double xn,double e,double M) {//start calculateXR
        double xR = 0;
